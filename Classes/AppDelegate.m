...
@synthesize window, iTunesURL, upgrade, boardView, congratulationsButton, prevImage, conImage, squareImage, prevBoard, prevBoardSol, toggleSwitch, pleaseWaitView, next, prev, nextBoard, generationThread, about, aboutCloseButton, aboutView, aboutContentView;
...
//	const int offsetX = 22;
	...
	if( [mainBundle objectForInfoDictionaryKey:@"Lite"] != nil) {
		lite = true;
		columns = 4;
		rows = 4;
		offsetX = 48;
	...
	*/
	...
	aboutContentView.delegate = self;
	...
	if( savedBoard != nil) {
		...
		if( boardView.board->MaxX != columns+1 || boardView.board->MaxY != rows+1) {
			boardView.board = nil;
			savedBoard = nil;
		}
	} 
	...
	if( lite) {
		self.upgrade = [[UIButton buttonWithType:UIButtonTypeCustom] initWithFrame:CGRectMake(0,21,100,100)];
		//[upgrade setFont:[UIFont systemFontOfSize:13]];
		//[upgrade setTitle:@"Upgrade to 5x6 puzzle" forState:0];
		upgrade.showsTouchWhenHighlighted = true;
		[upgrade setImage:[UIImage imageWithContentsOfFile:[mainBundle pathForResource:@"free" ofType:@"png"]] forState:0];
		//upgrade.lineBreakMode = UILineBreakModeWordWrap;
		[upgrade addTarget:self action:@selector(AboutButtonPressed) forControlEvents:UIControlEventTouchUpInside];
		[boardView addSubview:upgrade];
	}
	...
	self.prev = [[[UIButton buttonWithType:0] initWithFrame:CGRectMake(20,425,squaresize,squaresize)] autorelease];
	...
	self.next = [[[UIButton buttonWithType:0] initWithFrame:CGRectMake(squaresize*4+20,425,squaresize,squaresize)] autorelease];
	...
	self.pleaseWaitView = [[[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(squaresize*5.5+20-10, 430+squaresize/2-10, 20, 20)] autorelease];
...
- (BOOL)webView:(UIWebView*)webView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType {
	if( navigationType ==  UIWebViewNavigationTypeLinkClicked) {
		[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"itms://phobos.apple.com/WebObjects/MZStore.woa/wa/viewSoftware?id=319659123&mt=8&s=143441"]];
		//[self openReferralURL:[NSURL URLWithString:@"http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewSoftware?id=319659123&mt=8&s=143441"]];
	}
	return YES;
}
